...
let build_system_script =
  String.concat
    ...
    ; "import sys"
    ; ""
    ; "LOG = os.environ[\"PR3_LOG\"]"
    ; "TMP_ROOT = os.environ[\"PR3_TMP_ROOT\"]"
    ; ""
    ; "def read_atom(first):"
    ; "    length_digits = [first]"
    ; "    while True:"
...
let setup_fixture () =
  let tmp_root = setup_dir "ocamllsp-pr3-" in
...
let%test_unit "borrowed origin reuses the project context for external files" =
  ...
  Unix.putenv "PR3_LOG" log_path;
  Unix.putenv "PR3_TMP_ROOT" tmp_root;
