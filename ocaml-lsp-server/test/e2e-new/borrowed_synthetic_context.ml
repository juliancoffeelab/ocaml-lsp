...
let build_system_script =
  String.concat
    ...
    ; "import sys"
    ; ""
    ; "LOG = os.environ[\"PR5_LOG\"]"
    ; "TMP_ROOT = os.environ[\"PR5_TMP_ROOT\"]"
    ; ""
    ; "def read_atom(first):"
    ; "    length_digits = [first]"
    ; "    while True:"
...
let setup_fixture () =
  let tmp_root = setup_dir "ocamllsp-pr5-" in
  let project_root = Stdlib.Filename.concat tmp_root "project" in
  let origin_dir = Stdlib.Filename.concat project_root "app" in
  let target_dir =
    Stdlib.Filename.concat
      project_root
      "_build/_private/default/.pkg/smoke.0.0.1-digest/source"
  in
  Unix.mkdir project_root 0o700;
  Unix.mkdir origin_dir 0o700;
  Unix.mkdir (Stdlib.Filename.concat project_root "_build") 0o700;
  Unix.mkdir (Stdlib.Filename.concat project_root "_build/_private") 0o700;
  Unix.mkdir (Stdlib.Filename.concat project_root "_build/_private/default") 0o700;
  Unix.mkdir (Stdlib.Filename.concat project_root "_build/_private/default/.pkg") 0o700;
  Unix.mkdir
    (Stdlib.Filename.concat project_root "_build/_private/default/.pkg/smoke.0.0.1-digest")
    0o700;
  Unix.mkdir
    (Stdlib.Filename.concat
       project_root
       "_build/_private/default/.pkg/smoke.0.0.1-digest/source")
    0o700;
  let origin = Stdlib.Filename.concat origin_dir "main.ml" in
  let target = Stdlib.Filename.concat target_dir "smoke_dep.ml" in
  ...
  write_file origin "let _ = Smoke_dep.message\n";
  write_file target "let message = \"hi\"\n";
...
let%test_unit "borrowed origin reuses project context for synthetic pkg files" =
  let tmp_root, build_system, log_path, origin, target = setup_fixture () in
  Unix.putenv "PR5_LOG" log_path;
  Unix.putenv "PR5_TMP_ROOT" tmp_root;
  run_config_with_borrowed_context ~build_system ~origin ~target;
  let log = read_file_exn log_path in
  let expected =
    String.concat
      ~sep:""
      [ "path=$TMP/project/_build/_private/default/.pkg/smoke.0.0.1-digest/source/smoke_dep.ml;context=-\n"
      ; "path=$TMP/project/_build/_private/default/.pkg/smoke.0.0.1-digest/source/smoke_dep.ml;context=-\n"
      ; "path=$TMP/project/_build/_private/default/.pkg/smoke.0.0.1-digest/source/smoke_dep.ml;context=$TMP/project/app/main.ml\n"
      ]
  in
  if not (String.equal log expected)
  then
    failwith
      (Printf.sprintf "unexpected synthetic borrowed-context log\nexpected:\n%s\nactual:\n%s" expected log)
